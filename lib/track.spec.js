...
describe("track", () => {
    ...
    beforeEach(() => {
        ...
        configstoreSetStub = sandbox.stub(configstore_1.configstore, "set");
    ...
    describe("usageEnabled", () => {
        it("should return true if usage is enabled and IS_FIREBASE_CLI is true", () => {
            ...
            (0, chai_1.expect)(track.usageEnabled()).to.be.true;
        });
        ...
        it("should return false if usage is disabled", () => {
            ...
            (0, chai_1.expect)(track.usageEnabled()).to.be.false;
        });
    ...
    describe("track", () => {
        ...
        it("should include user properties in the request", async () => {
            ...
            await track.trackGA4("command_execution", { command_name: "test" });
            (0, chai_1.expect)(scope.isDone()).to.be.true;
            (0, chai_1.expect)(requestBody.user_properties.node_platform.value).to.equal(process.platform);
            (0, chai_1.expect)(requestBody.user_properties.node_version.value).to.equal(process.version);
            (0, chai_1.expect)(requestBody.user_properties.cli_version.value).to.equal(pkg.version);
        });
    ...
    describe("session", () => {
        ...
        it("should create a new client ID if one does not exist", () => {
            ...
            (0, chai_1.expect)(session).to.not.be.undefined;
            (0, chai_1.expect)(configstoreSetStub).to.have.been.calledOnce;
            (0, chai_1.expect)(configstoreSetStub.getCall(0).args[0]).to.equal("analytics-uuid");
        });
        it("should use an existing client ID if one exists", () => {
            configstoreGetStub.withArgs("analytics-uuid").returns("test-uuid");
            const session = track.cliSession();
            (0, chai_1.expect)(session?.clientId).to.equal("test-uuid");
            (0, chai_1.expect)(configstoreSetStub).to.not.have.been.called;
        });
        it("should cache the session object", () => {
            const session1 = track.cliSession();
            const session2 = track.cliSession();
            (0, chai_1.expect)(session1).to.equal(session2);
        });
        describe("debugMode", () => {
            it("should be true for @google.com accounts with tsconfig.json", () => {
                ...
                const session = track.cliSession();
                (0, chai_1.expect)(session?.debugMode).to.be.true;
            });
            it("should be false for non-@google.com accounts", () => {
                getGlobalDefaultAccountStub.returns({ user: { email: "test@example.com" } });
                const session = track.cliSession();
                (0, chai_1.expect)(session?.debugMode).to.be.false;
            });
        });
    });
});
//# sourceMappingURL=track.spec.js.map