...
describe("client", () => {
    ...
    describe("listLocations", () => {
        it("should list locations", async () => {
            ...
            (0, chai_1.expect)(locations).to.deep.equal(["us-central1", "us-east1"]);
            (0, chai_1.expect)(getStub).to.be.calledWith("/projects/project/locations");
        });
        it("should return empty list if no locations", async () => {
            getStub.resolves({ body: {} });
            const locations = await client.listLocations("project");
            (0, chai_1.expect)(locations).to.deep.equal([]);
        });
    });
    describe("Service methods", () => {
        it("getService", async () => {
            ...
            (0, chai_1.expect)(service).to.deep.equal({ name: "service" });
            (0, chai_1.expect)(getStub).to.be.calledWith("projects/p/locations/l/services/s");
        });
        it("listAllServices", async () => {
            ...
            (0, chai_1.expect)(services).to.deep.equal([{ name: "s1" }, { name: "s2" }]);
            (0, chai_1.expect)(getStub).to.be.calledWith("/projects/project/locations/-/services");
        });
        it("createService", async () => {
            ...
            (0, chai_1.expect)(service).to.deep.equal({ name: "service" });
            (0, chai_1.expect)(postStub).to.be.calledWith("/projects/p/locations/l/services", { name: "projects/p/locations/l/services/s" }, { queryParams: { service_id: "s" } });
            (0, chai_1.expect)(pollOperationStub).to.be.calledWith({
        ...
        it("createService returns undefined if service already exists", async () => {
            postStub.rejects(new error_1.FirebaseError("err", { status: 409 }));
            const service = await client.createService("p", "l", "s");
        ...
        it("deleteService", async () => {
            ...
            (0, chai_1.expect)(service).to.deep.equal({ name: "service" });
            (0, chai_1.expect)(deleteStub).to.be.calledWith("projects/p/locations/l/services/s", {
    ...
    describe("Schema methods", () => {
        it("getSchema", async () => {
            ...
            (0, chai_1.expect)(schema).to.deep.equal({ name: "schema" });
            (0, chai_1.expect)(getStub).to.be.calledWith("projects/p/locations/l/services/s/schemas/main");
        });
        it("getSchema with schemaId", async () => {
            getStub.resolves({ body: { name: "schema" } });
            const schema = await client.getSchema("projects/p/locations/l/services/s", "schemaId");
            (0, chai_1.expect)(schema).to.deep.equal({ name: "schema" });
            (0, chai_1.expect)(getStub).to.be.calledWith("projects/p/locations/l/services/s/schemas/schemaId");
        });
        it("getSchema returns undefined if not found", async () => {
            getStub.rejects(new error_1.FirebaseError("err", { status: 404 }));
            const schema = await client.getSchema("projects/p/locations/l/services/s");
        ...
        it("upsertSchema", async () => {
            ...
            (0, chai_1.expect)(schema).to.deep.equal({ name: "schema" });
            (0, chai_1.expect)(patchStub).to.be.calledWith("projects/p/locations/l/services/s/schemas/main", schemaToUpsert, { queryParams: { allowMissing: "true", validateOnly: "false" } });
        });
        it("deleteSchema", async () => {
            deleteStub.resolves({ body: { name: "op-name" } });
            pollOperationStub.resolves();
            await client.deleteSchema("projects/p/locations/l/services/s");
            (0, chai_1.expect)(deleteStub).to.be.calledWith("projects/p/locations/l/services/s/schemas/main");
        });
    });
    describe("Connector methods", () => {
        it("getConnector", async () => {
            ...
            (0, chai_1.expect)(connector).to.deep.equal({ name: "connector" });
            (0, chai_1.expect)(getStub).to.be.calledWith("projects/p/locations/l/services/s/connectors/c");
        });
        it("listConnectors", async () => {
            ...
            (0, chai_1.expect)(connectors).to.deep.equal([{ name: "c1" }, { name: "c2" }]);
        });
        it("upsertConnector", async () => {
            patchStub.resolves({ body: { name: "op-name" } });
            pollOperationStub.resolves({ name: "connector" });
            const connectorToUpsert = {
                name: "projects/p/locations/l/services/s/connectors/c",
                source: {},
            };
            const connector = await client.upsertConnector(connectorToUpsert);
            (0, chai_1.expect)(connector).to.deep.equal({ name: "connector" });
            (0, chai_1.expect)(patchStub).to.be.calledWith("projects/p/locations/l/services/s/connectors/c?allow_missing=true", connectorToUpsert);
        });
        it("deleteConnector", async () => {
            deleteStub.resolves({ body: { name: "op-name" } });
            pollOperationStub.resolves();
            await client.deleteConnector("projects/p/locations/l/services/s/connectors/c");
            (0, chai_1.expect)(deleteStub).to.be.calledWith("projects/p/locations/l/services/s/connectors/c");
        });
    });
});
