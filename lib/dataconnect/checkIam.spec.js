...
describe("grantRolesToCloudSqlServiceAccount", () => {
    ...
    let mergeBindingsStub;
    let printManualIamConfigStub;
    beforeEach(() => {
    ...
    it("should grant roles to the Cloud SQL service account", async () => {
        ...
        getInstanceStub.resolves(instance);
        getIamPolicyStub.resolves(policy);
        mergeBindingsStub.returns(true);
        setIamPolicyStub.resolves();
        await (0, checkIam_1.grantRolesToCloudSqlServiceAccount)("project", "instance", ["role1", "role2"]);
    ...
    it("should throw an error if setting the IAM policy fails", async () => {
        ...
        await expect((0, checkIam_1.grantRolesToCloudSqlServiceAccount)("project", "instance", ["role1", "role2"])).to.be.rejectedWith(error_1.FirebaseError, "Unable to make required IAM policy changes.");
