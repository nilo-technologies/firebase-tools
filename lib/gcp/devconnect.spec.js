...
const devconnect = require("./devConnect");
describe("developer connect", () => {
    ...
    function mockConnection(id, createTime) {
        return {
    ...
    describe("createConnection", () => {
        it("ensures githubConfig is FIREBASE", async () => {
            ...
            (0, chai_1.expect)(post).to.be.calledWith(connectionsRequestPath, { githubConfig: { githubApp: "FIREBASE" } }, { queryParams: { connectionId } });
        });
    });
    describe("listConnections", () => {
        it("interates through all pages and returns a single list", async () => {
            ...
            (0, chai_1.expect)(get).callCount(3);
            (0, chai_1.expect)(conns).to.deep.equal([firstConnection, secondConnection, thirdConnection]);
        });
    });
    describe("listAllLinkableGitRepositories", () => {
        it("interates through all pages and returns a single list", async () => {
            ...
            const conns = await devconnect.listAllLinkableGitRepositories(projectId, location, connectionId);
            (0, chai_1.expect)(get).callCount(3);
            (0, chai_1.expect)(conns).to.deep.equal([firstRepo, secondRepo, thirdRepo]);
        });
    });
    describe("listAllBranches", () => {
        it("interates through all pages and returns a single list and map", async () => {
            ...
            const branches = await devconnect.listAllBranches("/projects/blah/locations/us-central1/connections/blah");
            (0, chai_1.expect)(get).callCount(3);
            (0, chai_1.expect)(branches).to.deep.equal(new Set([firstBranch, secondBranch, thirdBranch]));
        });
        describe("sortConnectionsByCreateTime", () => {
            it("sorts the list of connections from earliest to latest", () => {
                ...
                (0, chai_1.expect)(devconnect.sortConnectionsByCreateTime(connections)).to.deep.equal([
                    sixthConnection,
                    thirdConnection,
                    fifthConnection,
                    secondConnection,
    ...
    describe("extractGitRepositoryLinkComponents", () => {
        it("correctly extracts the connection and git repository link ID", () => {
            (0, chai_1.expect)(devconnect.extractGitRepositoryLinkComponents(gitRepoLinkPath)).to.deep.equal({
...
//# sourceMappingURL=devconnect.spec.js.map