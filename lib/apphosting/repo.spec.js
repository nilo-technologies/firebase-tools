...
function mockReposWithRandomUris(n) {
    const repos = [];
    for (let i = 0; i < n; i++) {
...
describe("composer", () => {
    describe("connect GitHub repo", () => {
        ...
        let fetchLinkableRepositoriesStub;
        ...
        it("creates a connection if it doesn't exist", async () => {
            ...
            (0, chai_1.expect)(createConnectionStub).to.be.calledWith(projectId, location, connectionId);
        });
        it("checks if secret manager admin role is granted for cloud build P4SA when creating an oauth connection", async () => {
            ...
            (0, chai_1.expect)(serviceAccountHasRolesStub).to.be.calledWith(projectId, `service-${projectId}@gcp-sa-cloudbuild.iam.gserviceaccount.com`, ["roles/secretmanager.admin"], true);
        });
        it("creates repository if it doesn't exist", async () => {
            ...
            await repo.getOrCreateRepository(projectId, location, connectionId, repos.repositories[0].remoteUri);
            (0, chai_1.expect)(createRepositoryStub).to.be.calledWith(projectId, location, connectionId, "test-repo0", repos.repositories[0].remoteUri);
        });
        it("re-uses existing repository it already exists", async () => {
            ...
            const r = await repo.getOrCreateRepository(projectId, location, connectionId, repos.repositories[0].remoteUri);
    ...
    describe("parseConnectionName", () => {
        it("should parse valid connection name", () => {
            ...
            (0, chai_1.expect)(repo.parseConnectionName(str)).to.deep.equal(expected);
        });
    ...
    describe("fetchAllRepositories", () => {
        ...
        it("should fetch all repositories from multiple pages", async () => {
            ...
            (0, chai_1.expect)(repos.length).to.equal(20);
            (0, chai_1.expect)(Object.keys(remoteUriToConnection).length).to.equal(20);
        });
        it("should fetch all linkable repositories from multiple connections", async () => {
            ...
            (0, chai_1.expect)(remoteUriToConnection).to.deep.equal({
