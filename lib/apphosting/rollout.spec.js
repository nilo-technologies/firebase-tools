...
const githubConnections = require("../apphosting/githubConnections");
...
describe("apphosting rollouts", () => {
    ...
    let getBackend;
    let getRepoDetailsFromBackendStub;
    let listAllBranchesStub;
    let getGitHubBranchStub;
    let getGitHubCommitStub;
    let getNextRolloutIdStub;
    let createBuildStub;
    let createRolloutStub;
    let pollOperationStub;
    let promptGitHubBranchStub;
    let sleepStub;
    beforeEach(() => {
    ...
    describe("apphosting rollouts", () => {
        ...
        describe("createRollout", () => {
            it("should create a new rollout from user-specified branch", async () => {
                ...
                await (0, rollout_1.createRollout)(backendId, projectId, branchId, undefined, true);
                (0, chai_1.expect)(createBuildStub).to.be.called;
                (0, chai_1.expect)(createRolloutStub).to.be.called;
                (0, chai_1.expect)(pollOperationStub).to.be.called;
            });
            it("should create a new rollout from user-specified commit", async () => {
                ...
                pollOperationStub.onFirstCall().resolves(rollout);
                pollOperationStub.onSecondCall().resolves(build);
                await (0, rollout_1.createRollout)(backendId, projectId, undefined, commitSha, true);
                (0, chai_1.expect)(createBuildStub).to.be.called;
                (0, chai_1.expect)(createRolloutStub).to.be.called;
                (0, chai_1.expect)(pollOperationStub).to.be.called;
            });
            it("should prompt user for a branch if branch or commit ID is not specified", async () => {
                ...
                pollOperationStub.onFirstCall().resolves(rollout);
                pollOperationStub.onSecondCall().resolves(build);
                await (0, rollout_1.createRollout)(backendId, projectId, undefined, undefined, false);
                (0, chai_1.expect)(promptGitHubBranchStub).to.be.called;
                (0, chai_1.expect)(createBuildStub).to.be.called;
                (0, chai_1.expect)(createRolloutStub).to.be.called;
                (0, chai_1.expect)(pollOperationStub).to.be.called;
            });
            it("should throw an error if GitHub branch is not found", async () => {
                ...
                getRepoDetailsFromBackendStub.resolves(repoLinkDetails);
                listAllBranchesStub.resolves(branches);
                await (0, chai_1.expect)((0, rollout_1.createRollout)(backendId, projectId, "invalid-branch", undefined, true)).to.be.rejectedWith(/Unrecognized git branch/);
            });
            it("should throw an error if GitHub commit is not found", async () => {
                getBackend.resolves(backend);
                getRepoDetailsFromBackendStub.resolves(repoLinkDetails);
                getGitHubCommitStub.rejects(new error_1.FirebaseError("error", { status: 422 }));
                await (0, chai_1.expect)((0, rollout_1.createRollout)(backendId, projectId, undefined, commitSha, true)).to.be.rejectedWith(/Unrecognized git commit/);
            });
            it("should throw an error if --force flag is specified but --git-branch and --git-commit are missing", async () => {
                getBackend.resolves(backend);
                getRepoDetailsFromBackendStub.resolves(repoLinkDetails);
                await (0, chai_1.expect)((0, rollout_1.createRollout)(backendId, projectId, undefined, undefined, true)).to.be.rejectedWith(/Failed to create rollout with --force option/);
            });
        });
        describe("orchestrateRollout", () => {
            ...
            it("should retry createRollout call on HTTP 400 errors", async () => {
                getNextRolloutIdStub.resolves(buildAndRolloutId);
                createBuildStub.resolves(buildOp);
                createRolloutStub.onFirstCall().rejects(new error_1.FirebaseError("error", { status: 400 }));
                createRolloutStub.resolves(rolloutOp);
                pollOperationStub.onFirstCall().resolves(rollout);
                pollOperationStub.onSecondCall().resolves(build);
                sleepStub.resolves();
                ...
                (0, chai_1.expect)(createBuildStub).to.be.called;
                (0, chai_1.expect)(createRolloutStub).to.be.calledThrice;
                (0, chai_1.expect)(pollOperationStub).to.be.called;
            });
        });
    });
});
//# sourceMappingURL=rollout.spec.js.map