...
const parseTestFiles_1 = require("./parseTestFiles");
...
describe("parseTestFiles", () => {
    ...
    function writeFile(filename, content) {
        const file = (0, node_path_1.join)(tempdir.name, filename);
        fs.writeFileSync(file, content);
    }
    describe("parsing", () => {
        it("throws an error for invalid targetUri", async () => {
            writeFile("my_test.yaml", (0, yaml_1.stringify)({
                defaultConfig: { route: "/mypage" },
                tests: [{ displayName: "my test", steps: [{ goal: "click a button" }] }],
            }));
            await (0, chai_1.expect)((0, parseTestFiles_1.parseTestFiles)(tempdir.name, "foo.com")).to.be.rejectedWith(error_1.FirebaseError, "Invalid URL");
        });
        it("ignores invalid files", async () => {
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "http://www.foo.com");
            (0, chai_1.expect)(tests).to.eql([
                {
                    testCase: {
                        id: undefined,
        ...
        it("parses the sample test case file", async () => {
            writeFile("smoke_test.yaml", (0, templates_1.readTemplateSync)("init/apptesting/smoke_test.yaml"));
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "http://www.foo.com");
            (0, chai_1.expect)(tests).to.eql([
                {
                    testCase: {
        ...
        it("parses multiple test case files", async () => {
            writeFile("my_test.yaml", (0, yaml_1.stringify)({ tests: [{ displayName: "my test", steps: [{ goal: "click a button" }] }] }));
            writeFile("my_test2.yaml", (0, yaml_1.stringify)({
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com");
            (0, chai_1.expect)(tests).to.eql([
                {
                    testCase: {
                        id: undefined,
    ...
    describe("filtering", () => {
        function createBasicTest(displayNames) {
        ...
        async function getTestCaseNames(filenameFilter = "", testCaseFilter = "") {
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com", filenameFilter, testCaseFilter);
        ...
        it("returns an empty list if no match", async () => {
            ...
            (0, chai_1.expect)(await getTestCaseNames("yyy")).to.eql([]);
        });
        it("filters on filename", async () => {
            writeFile("aaa", createBasicTest(["axx", "ayy", "azz"]));
            writeFile("bbb", createBasicTest(["bxx", "byy", "bzz"]));
            (0, chai_1.expect)(await getTestCaseNames("aaa")).to.eql(["axx", "ayy", "azz"]);
        });
        it("filters on test case name", async () => {
            writeFile("aaa", createBasicTest(["axx", "ayy", "azz"]));
            writeFile("bbb", createBasicTest(["bxx", "byy", "bzz"]));
            (0, chai_1.expect)(await getTestCaseNames("", ".xx")).to.eql(["axx", "bxx"]);
        });
        it("filters on filename and test case name", async () => {
            writeFile("aaa", createBasicTest(["axx", "ayy", "azz"]));
            writeFile("bbb", createBasicTest(["bxx", "byy", "bzz"]));
            (0, chai_1.expect)(await getTestCaseNames("a$", "xx")).to.eql(["axx"]);
        });
    });
    describe("prerequisite test cases", () => {
        it("merges the steps from the prerequisite test case", async () => {
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com");
            (0, chai_1.expect)(tests.length).to.equal(2);
            const secondTest = tests[1];
            (0, chai_1.expect)(secondTest.testCase.steps).to.eql([
        ...
        it("throws an error for a non-existent prerequisite test case", async () => {
            ...
            await (0, chai_1.expect)((0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com")).to.be.rejectedWith(error_1.FirebaseError, "Invalid prerequisiteTestCaseId. There is no test case with id my-first-test");
        });
        it("handles an undefined prerequisite test case id", async () => {
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com");
            (0, chai_1.expect)(tests.length).to.equal(1);
            (0, chai_1.expect)(tests[0].testCase.steps).to.eql([{ goal: "do something" }]);
        });
        it("works correctly with filtering", async () => {
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com", "", "my second test");
            (0, chai_1.expect)(tests.length).to.equal(1);
        ...
        it("works correctly with multiple levels of prerequisites", async () => {
            ...
            const tests = await (0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com");
            (0, chai_1.expect)(tests.length).to.equal(3);
            const thirdTest = tests[2];
            (0, chai_1.expect)(thirdTest.testCase.steps).to.eql([
                { goal: "do something first" },
                { goal: "do something second" },
        ...
        it("throws error if there is a circular depedency", async () => {
            ...
            await (0, chai_1.expect)((0, parseTestFiles_1.parseTestFiles)(tempdir.name, "https://www.foo.com")).to.be.rejectedWith(error_1.FirebaseError, "Detected a cycle in prerequisite test cases.");
        });
    });
});
//# sourceMappingURL=parseTestFiles.spec.js.map