...
describe("Queue Test", () => {
    it("should create an empty task queue", () => {
        const taskQueue = new taskQueue_1.Queue();
        (0, chai_1.expect)(taskQueue).to.not.be.null;
    });
    ...
    it("should dequeue an element to a task queue", () => {
        ...
        (0, chai_1.expect)(second).to.eq(2);
    });
    ...
    it("should properly remove items from a queue", () => {
        ...
        const first = taskQueue.dequeue();
        const second = taskQueue.dequeue();
        (0, chai_1.expect)(first).to.eq(2);
        (0, chai_1.expect)(second).to.eq(4);
        (0, chai_1.expect)(() => taskQueue.dequeue()).to.throw("Trying to dequeue from an empty queue");
    });
    it("should error when trying to peek or remove from an empty task queue", () => {
        const taskQueue = new taskQueue_1.Queue();
        (0, chai_1.expect)(() => taskQueue.peek()).to.throw("Trying to peek into an empty queue");
        (0, chai_1.expect)(() => taskQueue.dequeue()).to.throw("Trying to dequeue from an empty queue");
    });
    ...
    it("should be able to peek into a queue", () => {
        const taskQueue = new taskQueue_1.Queue();
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(taskQueue.peek()).to.eq(1);
        (0, chai_1.expect)(taskQueue.peek()).to.eq(1);
    });
    it("should only allow unique IDs", () => {
        const taskQueue = new taskQueue_1.Queue();
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(() => taskQueue.enqueue("1", 1)).to.throw("Queue IDs must be unique");
    });
    it("should error when trying to remove a non-existent item", () => {
        const taskQueue = new taskQueue_1.Queue();
        (0, chai_1.expect)(() => taskQueue.remove("1")).to.throw("Trying to remove a task that doesn't exist");
    });
    it("should be able to remove an item when it is the only thing in the queue", () => {
        ...
        taskQueue.remove("1");
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(taskQueue.dequeue()).to.eq(1);
    });
    it("should properly determine if the queue is empty", () => {
        const taskQueue = new taskQueue_1.Queue();
        (0, chai_1.expect)(taskQueue.isEmpty()).to.eq(true);
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(taskQueue.isEmpty()).to.eq(false);
        taskQueue.dequeue();
        (0, chai_1.expect)(taskQueue.isEmpty()).to.eq(true);
    });
    it("should report the correct size", () => {
        ...
        taskQueue.remove("1");
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(taskQueue.size()).to.eq(1);
    });
    it("should error if at capacity", () => {
        const taskQueue = new taskQueue_1.Queue(1);
        taskQueue.enqueue("1", 1);
        (0, chai_1.expect)(() => taskQueue.enqueue("2", 2)).to.throw("Queue has reached capacity");
    });
    it("should return all items", () => {
        const taskQueue = new taskQueue_1.Queue();
        taskQueue.enqueue("1", 1);
        taskQueue.enqueue("2", 2);
        (0, chai_1.expect)(taskQueue.getAll()).to.deep.eq([1, 2]);
    });
});
describe("Task Queue", () => {
    ...
    const mockMetadata = {
        ...
        currentBackoff: 0,
        startTime: 0,
        status: taskQueue_1.TaskStatus.NOT_STARTED,
        lastRunTime: null,
        executionCount: 0,
        previousResponse: null,
    };
    const mockEmulatedTask = {
        task: mockTask,
    ...
    describe("Retry", () => {
        it("should update retried task status to not started", () => {
            const taskQueue = new taskQueue_1.TaskQueue(TEST_TASK_QUEUE_NAME, TEST_TASK_QUEUE_CONFIG);
            TEST_TASK.metadata.status = taskQueue_1.TaskStatus.RETRY;
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.status).to.be.eq(taskQueue_1.TaskStatus.NOT_STARTED);
        });
        it("should update retried task status to failed when max attempts are reached", () => {
            ...
            const taskQueue = new taskQueue_1.TaskQueue(TEST_TASK_QUEUE_NAME, config);
            TEST_TASK.metadata.status = taskQueue_1.TaskStatus.RETRY;
            TEST_TASK.metadata.currentAttempt = 11;
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.status).to.be.eq(taskQueue_1.TaskStatus.FAILED);
        });
        ...
        it("should double retry time", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(2);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(4);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(8);
        });
        it("should increment the attempt number", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentAttempt).to.be.eq(2);
        });
        it("shouldn't exceed the max backoff seconds", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(40);
        });
        it("should increase by a constant when doublings have maxxed", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(24);
            taskQueue.handleRetry(0);
            (0, chai_1.expect)(TEST_TASK.metadata.currentBackoff).to.be.eq(32);
        });
        it("should throw if task doesn't exist", () => {
            const taskQueue = new taskQueue_1.TaskQueue(TEST_TASK_QUEUE_NAME, TEST_TASK_QUEUE_CONFIG);
            taskQueue.setDispatch([TEST_TASK, null]);
            (0, chai_1.expect)(() => taskQueue.handleRetry(1)).to.throw("Trying to retry a nonexistent task");
        });
    });
    describe("Run Task", () => {
        it("should call the task url", () => {
            ...
            const res = taskQueue.runTask(0).then(() => {
                (0, chai_1.expect)(fetchStub).to.have.been.calledOnce.and.calledWith(TEST_TASK.task.httpRequest.url);
            });
            return res;
        });
        it("Should wait until the backoff time has elapsed", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            const res = taskQueue.runTask(0).then(() => {
                (0, chai_1.expect)(fetchStub).to.not.have.been.called;
            });
            return res;
        });
        ...
        it("should properly update metadata on success", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            const res = taskQueue.runTask(0).then(() => {
                (0, chai_1.expect)(TEST_TASK.metadata.status).to.be.eq(taskQueue_1.TaskStatus.FINISHED);
            });
            return res;
        });
        it("should properly update metadata on failure", () => {
            ...
            taskQueue.setDispatch([TEST_TASK]);
            const res = taskQueue.runTask(0).then(() => {
                (0, chai_1.expect)(TEST_TASK.metadata.status).to.be.eq(taskQueue_1.TaskStatus.RETRY);
                (0, chai_1.expect)(TEST_TASK.metadata.lastRunTime).to.be.eq(NOW);
            });
            return res;
        });
        it("should throw if task doesn't exist", async () => {
            const taskQueue = new taskQueue_1.TaskQueue(TEST_TASK_QUEUE_NAME, TEST_TASK_QUEUE_CONFIG);
            taskQueue.setDispatch([TEST_TASK, null]);
            await (0, chai_1.expect)(taskQueue.runTask(1)).to.be.rejectedWith("Trying to dispatch a nonexistent task");
        });
    });
    describe("enqueue", () => {
        it("should set the proper task uri", () => {
            ...
            (0, chai_1.expect)(task.httpRequest.url).to.be.eq(TEST_TASK_QUEUE_CONFIG.defaultUri);
        });
    });
    describe("Dispatch Tasks", () => {
        it("should move the first task in the queue to the dispatch", () => {
            ...
            (0, chai_1.expect)(taskQueue.getDispatch()[0].task).to.deep.eq(task);
        });
        it("should not dispatch tasks if the queue has no tokens", () => {
            ...
            taskQueue.dispatchTasks();
            (0, chai_1.expect)(taskQueue.getDispatch()[0]).to.be.eq(null);
        });
        it("should not dispatch tasks if the dispatch is full", () => {
            ...
            taskQueue.setTokens(1);
            taskQueue.dispatchTasks();
            (0, chai_1.expect)(taskQueue.getDispatch().map((et) => et?.task)).to.deep.eq([task1]);
        });
        it("should fill up empty dispatch slots", () => {
            ...
            taskQueue.setTokens(1);
            taskQueue.dispatchTasks();
            (0, chai_1.expect)(taskQueue.getDispatch().map((et) => et?.task)).to.deep.eq([
                TEST_TASK.task,
                task1,
                TEST_TASK.task,
            ]);
