...
const provision = require("../../../management/provisioning/provision");
...
describe("init ailogic", () => {
    ...
    describe("askQuestions", () => {
        ...
        it("should populate ailogic featureInfo with selected app ID", async () => {
            ...
            const mockSetup = { projectId: "test-project" };
            listFirebaseAppsStub.resolves(mockApps);
            selectStub.resolves(mockApps[0]);
            await init.askQuestions(mockSetup);
            (0, chai_1.expect)(mockSetup.featureInfo).to.have.property("ailogic");
            (0, chai_1.expect)(mockSetup.featureInfo?.ailogic).to.deep.equal({
                appId: "1:123456789:android:abcdef123456",
                displayName: "Test Android App",
            });
        });
        it("should throw error when no project ID is found", async () => {
            const mockSetup = {};
            await (0, chai_1.expect)(init.askQuestions(mockSetup)).to.be.rejectedWith("No project ID found. Please ensure you are in a Firebase project directory or specify a project.");
        ...
        it("should throw error when no apps are found", async () => {
            ...
            await (0, chai_1.expect)(init.askQuestions(mockSetup)).to.be.rejectedWith("No Firebase apps found in this project. Please create an app first using the Firebase Console or 'firebase apps:create'.");
            sinon.assert.calledWith(listFirebaseAppsStub, "test-project", apps_1.AppPlatform.ANY);
            sinon.assert.notCalled(selectStub);
        });
    });
    describe("actuate", () => {
        let setup;
        let parseAppIdStub;
        let provisionFirebaseAppStub;
        let getConfigFileNameStub;
        beforeEach(() => {
            setup = {
                config: {},
        ...
        it("should provision existing app successfully", async () => {
            ...
            (0, chai_1.expect)(setup.instructions).to.include("Firebase AI Logic has been enabled for existing ANDROID app: 1:123456789:android:abcdef123456");
            (0, chai_1.expect)(setup.instructions).to.include("Save the following content as google-services.json in your app's root directory:");
            (0, chai_1.expect)(setup.instructions).to.include(mockConfigContent);
        });
        it("should throw error if no project ID found", async () => {
            setup.projectId = undefined;
            await (0, chai_1.expect)(init.actuate(setup)).to.be.rejectedWith("AI Logic setup failed: No project ID found. Please ensure you are in a Firebase project directory or specify a project.");
        ...
        it("should handle provisioning errors gracefully", async () => {
            ...
            await (0, chai_1.expect)(init.actuate(setup)).to.be.rejectedWith("AI Logic setup failed: Provisioning API failed");
        });
        it("should include config file content in instructions for iOS", async () => {
            ...
            await init.actuate(setup);
            (0, chai_1.expect)(setup.instructions).to.include("Firebase AI Logic has been enabled for existing IOS app: 1:123456789:ios:abcdef123456");
            (0, chai_1.expect)(setup.instructions).to.include("Save the following content as GoogleService-Info.plist in your app's root directory:");
            (0, chai_1.expect)(setup.instructions).to.include(mockConfigContent);
        });
        it("should include platform placement guidance in instructions", async () => {
            ...
            getConfigFileNameStub.returns("google-services.json");
            await init.actuate(setup);
            (0, chai_1.expect)(setup.instructions).to.include("Place this config file in the appropriate location for your platform.");
        });
    });
});
