...
describe("simplifyReport", () => {
    it("should return report unchanged if groups is undefined", () => {
        ...
        const result = (0, reports_1.simplifyReport)(report);
...
describe("getReport", () => {
    ...
    it("should resolve with the response body on success", async () => {
        ...
        const result = await (0, reports_1.getReport)(reports_1.CrashlyticsReport.TOP_ISSUES, appId, { issueErrorTypes: [issueType] }, pageSize);
        expect(result).to.deep.equal(mockResponse);
        expect(nock.isDone()).to.be.true;
    });
    it("should throw a FirebaseError if the appId is invalid", async () => {
        const invalidAppId = "invalid-app-id";
        await expect((0, reports_1.getReport)(reports_1.CrashlyticsReport.TOP_ISSUES, invalidAppId, {})).to.be.rejectedWith(error_1.FirebaseError, "Unable to get the projectId from the AppId.");
    });
});
