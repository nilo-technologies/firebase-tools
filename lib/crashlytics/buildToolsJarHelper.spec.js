...
describe("buildToolsJarHelper", () => {
    ...
    describe("runBuildtoolsCommand", () => {
        ...
        it("should call spawn.sync with the correct arguments", () => {
            ...
            expect(spawnSyncStub).to.have.been.calledWith("java", ["-jar", jarFile, ...args, "-clientName", "firebase-cli;crashlytics-buildtools"], { stdio: "pipe" });
        });
        it("should use 'inherit' for stdio when debug is true", () => {
            ...
            expect(spawnSyncStub).to.have.been.calledWith("java", ["-jar", jarFile, ...args, "-clientName", "firebase-cli;crashlytics-buildtools"], { stdio: "inherit" });
        });
        it("should throw a FirebaseError on command failure", () => {
            spawnSyncStub.returns({ status: 1, stdout: "error output" });
            const jarFile = "my.jar";
            const args = ["arg1", "arg2"];
            expect(() => buildToolsJarHelper.runBuildtoolsCommand(jarFile, args, false)).to.throw(error_1.FirebaseError, /java command failed/);
        });
...
//# sourceMappingURL=buildToolsJarHelper.spec.js.map