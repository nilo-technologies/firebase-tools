...
const timer_1 = require("./release/timer");
exports.EVENTARC_SOURCE_ENV = "EVENTARC_CLOUD_EVENT_SOURCE";
async function prepare(context, options, payload) {
    const prepareTimer = new timer_1.Timer();
    logger_1.logger.info("[timing] functions: prepare: starting preparation");
    ...
    const apiTimer = new timer_1.Timer();
    ...
    logger_1.logger.info(`[timing] functions: prepare: ensured minimum APIs enabled (${apiTimer.stop()}ms)`);
    ...
    const loadCodebasesTimer = new timer_1.Timer();
    const wantBuilds = await loadCodebases(context.config, options, firebaseConfig, runtimeConfig, context.filters);
    logger_1.logger.info(`[timing] functions: prepare: loaded codebases (${loadCodebasesTimer.stop()}ms)`);
    ...
    const resolveBackendsTimer = new timer_1.Timer();
    ...
    logger_1.logger.info(`[timing] functions: prepare: resolved backends (${resolveBackendsTimer.stop()}ms)`);
    const prepareSourceTimer = new timer_1.Timer();
    context.sources = {};
    ...
    logger_1.logger.info(`[timing] functions: prepare: prepared source for upload (${prepareSourceTimer.stop()}ms)`);
    const fillDetailsTimer = new timer_1.Timer();
    payload.functions = {};
    const existingBackendTimer = new timer_1.Timer();
    const existingBackendResult = await backend.existingBackend(context);
    logger_1.logger.info(`[timing] functions: prepare: fetched existing backend (${existingBackendTimer.stop()}ms)`);
    const haveBackends = (0, functionsDeployHelper_1.groupEndpointsByCodebase)(wantBackends, backend.allEndpoints(existingBackendResult));
    for (const [codebase, wantBackend] of Object.entries(wantBackends)) {
    ...
    logger_1.logger.info(`[timing] functions: prepare: filled in details and validated endpoints (${fillDetailsTimer.stop()}ms)`);
    const enableApisTimer = new timer_1.Timer();
    ...
    logger_1.logger.info(`[timing] functions: prepare: enabled required APIs (${enableApisTimer.stop()}ms)`);
    ...
    const finalizeTimer = new timer_1.Timer();
    ...
    logger_1.logger.info(`[timing] functions: prepare: finalized IAM and secrets (${finalizeTimer.stop()}ms)`);
    ...
    logger_1.logger.info(`[timing] functions: prepare: total preparation time (${prepareTimer.stop()}ms)`);
}
