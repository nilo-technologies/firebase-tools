...
async function deploy(context, options, payload) {
    const deployTimer = new timer_1.Timer();
    logger_1.logger.info("[timing] functions: deploy: starting source upload");
    if (payload.extensions && context.extensions) {
        const extTimer = new timer_1.Timer();
        await (0, extensions_1.deploy)(context.extensions, options, payload.extensions);
        logger_1.logger.info(`[timing] functions: deploy: deployed extensions (${extTimer.stop()}ms)`);
    }
    if (payload.functions && context.config) {
        const iamCheckTimer = new timer_1.Timer();
        await (0, checkIam_1.checkHttpIam)(context, options, payload);
        logger_1.logger.info(`[timing] functions: deploy: checked HTTP IAM (${iamCheckTimer.stop()}ms)`);
        const uploadTimer = new timer_1.Timer();
        const uploads = [];
        for (const [codebase, { wantBackend, haveBackend }] of Object.entries(payload.functions)) {
            if (shouldUploadBeSkipped(context, wantBackend, haveBackend)) {
                logger_1.logger.info(`[timing] functions: deploy: skipping upload for codebase ${codebase} (unchanged)`);
                continue;
            }
        ...
        logger_1.logger.info(`[timing] functions: deploy: uploaded source (${uploadTimer.stop()}ms)`);
    }
    logger_1.logger.info(`[timing] functions: deploy: total deploy time (${deployTimer.stop()}ms)`);
}
