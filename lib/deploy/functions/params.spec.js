...
describe("resolveParams", () => {
    ...
    it("can resolve a CEL expression depending on the internal params", async () => {
        ...
        expect(input.getCall(2).args[0].default).to.eq("http://foo.appspot.com.storage.googleapis.com/");
    });
    ...
    it("does not throw in non-interactive mode if secret exists in cloud", async () => {
        const paramsToResolve = [{ name: "MY_SECRET", type: "secret" }];
        const getSecretMetadataStub = sinon.stub(secretManager, "getSecretMetadata").resolves({
            ...
            secretVersion: { versionId: "1", state: "ENABLED", secret: {} },
        });
    ...
    it("throws in non-interactive mode if secret is missing in cloud", async () => {
        ...
        await expect(params.resolveParams(paramsToResolve, fakeConfig, {}, true)).to.be.rejectedWith(error_1.FirebaseError, /In non-interactive mode but have no value for the secret/);
        getSecretMetadataStub.restore();
    });
});
