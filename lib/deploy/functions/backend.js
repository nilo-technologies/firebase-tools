...
async function loadExistingBackend(ctx) {
    const loadTimer = new timer_1.Timer();
    logger_1.logger.info("[timing] functions: backend: loading existing backend");
    ...
    const gcfV1Timer = new timer_1.Timer();
    ...
    logger_1.logger.info(`[timing] functions: backend: listed ${gcfV1Results.functions.length} GCFv1 functions (${gcfV1Timer.stop()}ms)`);
    if (experiments.isEnabled("functionsrunapionly")) {
        const runTimer = new timer_1.Timer();
        try {
            ...
            logger_1.logger.info(`[timing] functions: backend: listed ${runServices.length} Cloud Run services (${runTimer.stop()}ms)`);
        }
        catch (err) {
        ...
        const gcfV2Timer = new timer_1.Timer();
        ...
        logger_1.logger.info(`[timing] functions: backend: listed ${gcfV2Results.functions.length} GCFv2 functions (${gcfV2Timer.stop()}ms)`);
    }
    const totalEndpoints = allEndpoints(existingBackend).length;
    logger_1.logger.info(`[timing] functions: backend: loaded ${totalEndpoints} total existing endpoints (${loadTimer.stop()}ms)`);
