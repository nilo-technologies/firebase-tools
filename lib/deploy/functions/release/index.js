...
async function release(context, options, payload) {
    const releaseTimer = new timer_1.Timer();
    logger_1.logger.info("[timing] functions: release: starting release");
    if (context.extensions && payload.extensions) {
        ...
        logger_1.logger.info(`[timing] functions: release: released extensions (${extTimer.stop()}ms)`);
    }
    if (!context.config) {
        return;
    }
    ...
    const planningTimer = new timer_1.Timer();
    let plan = {};
    ...
    logger_1.logger.info(`[timing] functions: release: created deployment plan (${planningTimer.stop()}ms)`);
    ...
    const applyPlanTimer = new timer_1.Timer();
    const summary = await fab.applyPlan(plan);
    logger_1.logger.info(`[timing] functions: release: applied deployment plan (${applyPlanTimer.stop()}ms)`);
    ...
    logger_1.logger.info(`[timing] functions: release: total release time (${releaseTimer.stop()}ms)`);
}
