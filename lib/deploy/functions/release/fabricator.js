...
class Fabricator {
    ...
    async applyPlan(plan) {
        ...
        for (const changes of Object.values(plan)) {
            totalCreates += changes.endpointsToCreate.length;
            totalUpdates += changes.endpointsToUpdate.length;
            totalDeletes += changes.endpointsToDelete.length;
            totalSkips += changes.endpointsToSkip.length;
        }
        logger_1.logger.info(`[timing] functions: fabricator: applying plan with ${totalCreates} creates, ${totalUpdates} updates, ${totalDeletes} deletes, ${totalSkips} skips`);
    ...
    async applyChangeset(changes) {
        const changesetTimer = new timer_1.Timer();
        const deployResults = [];
        const handle = async (op, endpoint, fn) => {
            ...
            logger_1.logger.info(`[timing] functions: fabricator: ${op} ${endpoint.id} in ${endpoint.region} completed (${result.durationMs}ms)`);
        ...
        const createCount = changes.endpointsToCreate.length;
        const updateCount = changes.endpointsToUpdate.length;
        const skipCount = changes.endpointsToSkip.length;
        const deleteCount = changes.endpointsToDelete.length;
        if (createCount + updateCount + skipCount + deleteCount > 0) {
            logger_1.logger.info(`[timing] functions: fabricator: changeset has ${createCount} creates, ${updateCount} updates, ${skipCount} skips, ${deleteCount} deletes`);
        }
        ...
        if (upserts.length > 0) {
            const upsertTimer = new timer_1.Timer();
            await utils.allSettled(upserts);
            logger_1.logger.info(`[timing] functions: fabricator: all creates/updates completed (${upsertTimer.stop()}ms)`);
        }
        if (deployResults.find((r) => r.error)) {
            ...
            logger_1.logger.info(`[timing] functions: fabricator: changeset completed with errors (${changesetTimer.stop()}ms)`);
        ...
        if (deletes.length > 0) {
            const deleteTimer = new timer_1.Timer();
            await utils.allSettled(deletes);
            logger_1.logger.info(`[timing] functions: fabricator: all deletes completed (${deleteTimer.stop()}ms)`);
        }
        logger_1.logger.info(`[timing] functions: fabricator: changeset completed (${changesetTimer.stop()}ms)`);
        return deployResults;
    }
