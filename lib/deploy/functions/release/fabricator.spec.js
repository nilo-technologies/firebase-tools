...
describe("Fabricator", () => {
    ...
    describe("createV2Function", () => {
        ...
        it("handles failures to create a topic", async () => {
            ...
            await (0, chai_1.expect)(fab.createV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "create topic");
        });
        ...
        it("wraps errors thrown while creating channels", async () => {
            ...
            await (0, chai_1.expect)(fab.createV2Function(ep, new scraper.SourceTokenScraper())).to.eventually.be.rejectedWith(reporter.DeploymentError, "upsert eventarc channel");
        });
        it("throws on create function failure", async () => {
            gcfv2.createFunction.rejects(new Error("Server failure"));
            const ep = endpoint({ httpsTrigger: {} }, { platform: "gcfv2" });
            await (0, chai_1.expect)(fab.createV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "create");
            gcfv2.createFunction.resolves({ name: "op", done: false });
            poller.pollOperation.rejects(new Error("Fail whale"));
            await (0, chai_1.expect)(fab.createV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "create");
        });
        it("tries to grab new token on abort", async () => {
            ...
            await (0, chai_1.expect)(sc.getToken()).to.eventually.equal("magic token");
        });
        it("deletes broken function and retries on cloud run quota exhaustion", async () => {
            ...
            const ep = endpoint({ httpsTrigger: {} }, { platform: "gcfv2" });
            await fab.createV2Function(ep, new scraper.SourceTokenScraper());
            (0, chai_1.expect)(gcfv2.createFunction).to.have.been.calledTwice;
            (0, chai_1.expect)(gcfv2.deleteFunction).to.have.been.called;
        });
        it("throws on set invoker failure", async () => {
            gcfv2.createFunction.resolves({ name: "op", done: false });
            poller.pollOperation.resolves({ serviceConfig: { service: "service" } });
            ...
            const ep = endpoint({ httpsTrigger: {} }, { platform: "gcfv2" });
            await (0, chai_1.expect)(fab.createV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "set invoker");
        });
        describe("httpsTrigger", () => {
        ...
        describe("dataConnectGraphqlTrigger", () => {
            it("sets FDC P4SA invoker by default", async () => {
                ...
                await fab.createV2Function(ep, new scraper.SourceTokenScraper());
                (0, chai_1.expect)(run.setInvokerCreate).to.have.been.calledWith(ep.project, "service", [
            ...
            it("doesn't set private invoker on create", async () => {
                ...
                poller.pollOperation.resolves({ serviceConfig: { service: "service" } });
                run.setInvokerCreate.resolves();
                const ep = endpoint({ dataConnectGraphqlTrigger: { invoker: ["private"] } }, { platform: "gcfv2" });
                await fab.createV2Function(ep, new scraper.SourceTokenScraper());
                (0, chai_1.expect)(run.setInvokerCreate).to.not.have.been.called;
            });
        });
        ...
        describe("scheduledTrigger", () => {
            it("sets the default compute service account by default", async () => {
                ...
                poller.pollOperation.resolves({ serviceConfig: { service: "service" } });
                run.setInvokerCreate.resolves();
                const ep = endpoint({ scheduleTrigger: { schedule: "every 5 minutes" } }, { platform: "gcfv2" });
                await fab.createV2Function(ep, new scraper.SourceTokenScraper());
                (0, chai_1.expect)(run.setInvokerCreate).to.have.been.calledWith(ep.project, "service", [
            ...
            it("sets the an explicit service account", async () => {
                ...
                poller.pollOperation.resolves({ serviceConfig: { service: "service" } });
                run.setInvokerCreate.resolves();
                const ep = endpoint({ scheduleTrigger: { schedule: "every 5 minutes" } }, { platform: "gcfv2", serviceAccount: "sa@" });
                await fab.createV2Function(ep, new scraper.SourceTokenScraper());
                (0, chai_1.expect)(run.setInvokerCreate).to.have.been.calledWith(ep.project, "service", ["sa@"]);
            });
        });
        it("doesn't set invoker on non-http functions", async () => {
            gcfv2.createFunction.resolves({ name: "op", done: false });
    ...
    describe("updateV2Function", () => {
        it("throws on update function failure", async () => {
            ...
            const ep = endpoint({ httpsTrigger: {} }, { platform: "gcfv2" });
            await (0, chai_1.expect)(fab.updateV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "update");
        ...
        it("throws on set invoker failure", async () => {
            ...
            const ep = endpoint({ httpsTrigger: { invoker: ["private"] } }, { platform: "gcfv2" });
            await (0, chai_1.expect)(fab.updateV2Function(ep, new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "set invoker");
        });
        ...
        it("sets explicit invoker on dataConnectGraphqlTrigger", async () => {
            ...
            await fab.updateV2Function(ep, new scraper.SourceTokenScraper());
            (0, chai_1.expect)(run.setInvokerUpdate).to.have.been.calledWith(ep.project, "service", [
                "custom@",
                `service-${fab.projectNumber}@gcp-sa-firebasedataconnect.iam.gserviceaccount.com`,
            ]);
        });
    ...
    describe("upsertScheduleV2", () => {
        ...
        it("upserts schedules with explicit service account", async () => {
            const withSA = (0, core_1.deepCopy)(ep);
            ...
            (0, chai_1.expect)(schedule.httpTarget?.oidcToken?.serviceAccountEmail).to.equal(withSA.serviceAccount);
        });
        it("wraps errors", async () => {
            scheduler.createOrReplaceJob.rejects(new Error("Fail"));
    ...
    describe("createEndpoint", () => {
        it("creates v1 functions", async () => {
            ...
            await fab.createEndpoint(ep, new scraper.SourceTokenScraper(), new scraper.SourceTokenScraper());
        ...
        it("aborts for failures midway", async () => {
            ...
            await (0, chai_1.expect)(fab.createEndpoint(ep, new scraper.SourceTokenScraper(), new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "set invoker");
            (0, chai_1.expect)(createV1Function).is.calledOnce;
    ...
    describe("updateEndpoint", () => {
        it("updates v1 functions", async () => {
            ...
            await fab.updateEndpoint({ endpoint: ep }, new scraper.SourceTokenScraper(), new scraper.SourceTokenScraper());
        ...
        it("aborts for failures midway", async () => {
            ...
            await (0, chai_1.expect)(fab.updateEndpoint({ endpoint: ep }, new scraper.SourceTokenScraper(), new scraper.SourceTokenScraper())).to.be.rejectedWith(reporter.DeploymentError, "set invoker");
            (0, chai_1.expect)(updateV1Function).is.calledOnce;
            (0, chai_1.expect)(setTrigger).is.not.called;
        });
        it("can delete and create", async () => {
            ...
            const createV2Function = sinon.stub(fab, "createV2Function");
            createV2Function.resolves();
            await fab.updateEndpoint(update, new scraper.SourceTokenScraper(), new scraper.SourceTokenScraper());
    ...
    describe("applyRegionalUpdates", () => {
        ...
        it("handles errors and wraps them in results", async () => {
            ...
            (0, chai_1.expect)(results[0].error?.message).to.match(/create function/);
        });
    });
    ...
    it("does not delete if there are upsert errors", async () => {
        ...
        (0, chai_1.expect)(result?.error).to.be.instanceOf(reporter.AbortedDeploymentError);
        (0, chai_1.expect)(result?.durationMs).to.equal(0);
    });
    ...
    describe("applyPlan", () => {
        it("fans out to regions", async () => {
            ...
            (0, chai_1.expect)(ep1Result?.error).to.be.instanceOf(reporter.DeploymentError);
            (0, chai_1.expect)(ep1Result?.error?.message).to.match(/create function/);
            const ep2Result = summary.results.find((r) => r.endpoint.region === ep2.region);
            (0, chai_1.expect)(ep2Result?.error).to.be.instanceOf(reporter.DeploymentError);
            (0, chai_1.expect)(ep2Result?.error?.message).to.match(/delete function/);
        });
    });
});
