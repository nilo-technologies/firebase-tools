...
function extension(latest, latestApproved) {
    return {
...
describe("Extensions Deployment Planner", () => {
            ...
            it(c.description, () => {
                if (!c.err) {
                    (0, chai_1.expect)(planner.resolveVersion({
                        ...
                        extensionId: "test",
                        version: c.in,
                    }, extension("0.2.0", "0.1.1"))).to.eventually.equal(c.out);
                }
                else {
                    (0, chai_1.expect)(planner.resolveVersion({
                        publisherId: "test",
                        extensionId: "test",
                        version: c.in,
                    }, extension("0.2.0", "0.1.1"))).to.eventually.be.rejected;
                }
            });
        }
    });
    describe("have", () => {
        ...
        const INSTANCE_WITH_SDK_LABELS = JSON.parse(JSON.stringify(INSTANCE_WITH_EVENTS));
    ...
    describe("haveDynamic", () => {
        ...
        INSTANCE_SPEC_WITH_EMPTY_EVENTS_CONFIG.eventarcChannel = "";
        INSTANCE_SPEC_WITH_EMPTY_EVENTS_CONFIG.allowedEventTypes = [];
        const INSTANCE_WITHOUT_SDK_LABELS = JSON.parse(JSON.stringify(INSTANCE_WITH_EVENTS));
        ...
        for (const c of cases) {
            it(c.description, async () => {
                ...
                (0, chai_1.expect)(haveDynamic).to.have.same.deep.members(c.instanceSpecs);
                listInstancesStub.restore();
            });
        }
    });
