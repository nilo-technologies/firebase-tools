...
describe("convertConfig", () => {
    let setRewriteTagsStub;
    let wasPinTagsEnabled;
    before(() => {
    ...
    for (const { name, input, existingBackend, want, functionsPayload } of tests) {
        it(name, async () => {
            const context = {
                ...
                existingBackendPromise: new Promise((resolve) => resolve(existingBackend || backend.empty())),
    ...
    describe("rewrites errors", () => {
        ...
        beforeEach(() => {
            existingBackendStub = sinon
                ...
                .rejects(new error_1.FirebaseError("Some permissions 403 error (that should be caught)", { status: 403 }));
        });
        afterEach(() => {
