...
export class Fabricator {
  ...
  async applyPlan(plan: planner.DeploymentPlan): Promise<reporter.Summary> {
    ...
    logger.info(
      `[timing] functions: fabricator: applying plan with ${totalCreates} creates, ${totalUpdates} updates, ${totalDeletes} deletes, ${totalSkips} skips`,
    );
  ...
  async applyChangeset(changes: planner.Changeset): Promise<Array<reporter.DeployResult>> {
    const changesetTimer = new Timer();
    ...
    const handle = async (
      ...
      logger.info(
        `[timing] functions: fabricator: ${op} ${endpoint.id} in ${endpoint.region} completed (${result.durationMs}ms)`,
      );
    ...
    if (createCount + updateCount + skipCount + deleteCount > 0) {
      logger.info(
        `[timing] functions: fabricator: changeset has ${createCount} creates, ${updateCount} updates, ${skipCount} skips, ${deleteCount} deletes`,
      );
    }
    ...
    if (upserts.length > 0) {
      ...
      logger.info(
        `[timing] functions: fabricator: all creates/updates completed (${upsertTimer.stop()}ms)`,
      );
    }
    ...
    if (deployResults.find((r) => r.error)) {
      ...
      logger.info(`[timing] functions: fabricator: changeset completed with errors (${changesetTimer.stop()}ms)`);
    ...
    if (deletes.length > 0) {
      ...
      logger.info(`[timing] functions: fabricator: all deletes completed (${deleteTimer.stop()}ms)`);
    }

    logger.info(`[timing] functions: fabricator: changeset completed (${changesetTimer.stop()}ms)`);
    return deployResults;
  }
